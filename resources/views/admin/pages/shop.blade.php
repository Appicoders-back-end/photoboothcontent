...
@section('content')
    <section class="wrapper">
        ...
        @endif
        <h6>Update Shop Page</h6>
        <form class="needs-validation" action="{{route('admin.storeShopPage')}}" method="POST"
              enctype="multipart/form-data">
            @csrf
            <input type="hidden" name="id" value="{{$shop->id}}">
            @if($content)
                @if(isset($content->shopImg))
                    <input type="hidden" name="old_image" value="{{ $content->shopImg }}">
                @endif
            @endif
            <div class="row">
                <div class="col-lg-12">
                    <section class="card">
                        ...
                        <div class="card-body">
                            <div class="form-row">
                                ...
                                <div class="col-md-12 mb-3">
                                    <label>Image</label>
                                    <input type="file" class="dropify" name="shop_image"
                                           data-max-file-size="10M"
                                           data-allowed-file-extensions="jpg jpeg png"
                                           data-show-remove="false"
                                           @if(isset($content->shopImg))
                                               data-default-file="{{ url('/') . '/' . $content->shopImg }}"
                                           @endif
                                    />
                                </div>
                                ...
                                <div class="col-md-12 mb-3">
                                    ...
                                    <input type="text" class="form-control" id="button_text"
                                           ...
                                           value="{{old('hero_button_text', $content->hero_button_text ?? null)}}">
                                </div>
                            </div>
                        </div>
                    </section>
