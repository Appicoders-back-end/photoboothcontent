...
@section('content')
    <section class="wrapper">
        ...
        <form class="needs-validation" action="{{route('admin.storeContentPage')}}" method="POST"
              enctype="multipart/form-data">
            @csrf
            <input type="hidden" name="id" value="{{$content_->id}}">
            <input type="hidden" name="old_image" value="{{ ($content) ? ($content->contentImg) ? $content->contentImg : '' : '' }}">
            <div class="row">
                <div class="col-lg-12">
                    <section class="card">
                        ...
                        <div class="card-body">
                            <div class="form-row">
                                ...
                                <div class="col-md-12 mb-3">
                                    <label>Image</label>
                                    <input type="file" class="dropify" name="content_image"
                                           data-max-file-size="10M"
                                           data-allowed-file-extensions="jpg jpeg png"
                                           data-show-remove="false"
                                           @if(isset($content->contentImg))
                                               data-default-file="{{ url('/') . '/' . $content->contentImg }}"
                                           @endif
                                    />
                                </div>

                                <div class="col-md-12 mb-3">
                                    <label for="content_button_text">Button Text</label>
                                    <input type="text" class="form-control" id="content_button_text"
                                           name="content_button_text"
                                           placeholder="Enter Button Text"
                                           value="{{old('content_button_text', $content->content_button_text ?? null)}}">
                                </div>
                            </div>
                        </div>
                    </section>
